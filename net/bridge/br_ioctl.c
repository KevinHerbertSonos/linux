...
/* called with RTNL */
static unsigned char get_any_forwarding(struct net_bridge *br)
{
	struct net_bridge_port *p;
...
static int old_dev_ioctl(struct net_device *dev, struct ifreq *rq, int cmd)
{
	...
	case BRCTL_MOD_PORT_ADDR:
		...
		if (copy_from_user(oldaddr, (void *)args[1], ETH_ALEN)) {
				ret = -EFAULT;
		}

		if (copy_from_user(newaddr, (void *)args[2], ETH_ALEN)) {
			ret = -EFAULT;
		}
	...
	case BRCTL_MOD_PORT_DEV:
		...
		dev = dev_get_by_index(&init_net, args[2]);
		if (dev == NULL) {
			return -EINVAL;
		}
	...
	case BRCTL_ADD_UPLINK:
        ...
        case BRCTL_GET_P2P_TUNNEL_STATES:
        {
                ...
                if (copy_to_user((void *)args[2], 
                                 state_data, sizeof(state_data)) ||
                    copy_to_user((void *)args[3],
	...
	case BRCTL_SET_P2P_DIRECT_ENABLED: {
		...
		if (copy_from_user(direct_addr, (void *)(args[3]), ETH_ALEN))
			ret = -EFAULT;
	...
	case BRCTL_GET_ROUTING_CAPABILITIES: {
		...
		if (copy_to_user((void *)args[1], &rc, sizeof(rc))) {
			ret = -EFAULT;
		}
	...
	case BRCTL_SET_STATIC_MAC: {
		...
		if (copy_from_user(mac, (void *)args[1], ETH_ALEN))                
			ret = -EFAULT;
	...
	case BRCTL_SET_UPLINK_MODE: {
		br_set_uplink_mode(br, (args[1] != 0));
                return 0;
	}
	...
	case BRCTL_GET_STATS: {
		...
		if (copy_to_user((void *)args[1], &stats, sizeof(stats))) {
			ret = -EFAULT;
		}
		return ret;
	}
...
int br_ioctl_deviceless_stub(struct net *dev, unsigned int cmd, void __user *uarg)
{
	switch (cmd) {
	case SIOCGIFBR:
	case SIOCSIFBR:
