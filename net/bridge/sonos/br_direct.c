/* br_direct.c - Sonos Direct Routing Extensions
					    ...
					    struct sk_buff *skb))
{
	 ...
	 */
	BUILD_BUG_ON(sizeof(p_dir->direct_last_stp_time) != sizeof(jiffies));
	if (time_after(jiffies, p_dir->direct_last_stp_time + BR_DIRECT_STP_TIME)) {
