/* br_mcast.c - Sonos Multicast Extensions
...
#define GROUP_JOIN_IP_TAG  0x5A
#define GROUP_JOIN_PADDING 0xA5
...
static __inline__ uint32_t br_get_bridge_ip(struct net_bridge *br)
{
    uint32_t ip = 0;
...
void br_mcast_transmit_grouplist(struct net_bridge *br)
{
    ...
    size = 2 * ETH_ALEN + 2 + 1 + 6 * br->num_mcast_groups + 1 + 4;
    if (size < 60)
        size = 60;
    ...
    memcpy(skb_mac_header(skb), rincon_gmp_addr, ETH_ALEN);
    memcpy(skb_mac_header(skb) + ETH_ALEN, br->dev->dev_addr, ETH_ALEN);
    memcpy(skb_mac_header(skb) + 2*ETH_ALEN, &nbe_protocol, 2);
    ...
    *(data++) = (unsigned char)(br->num_mcast_groups);
    ...
    br_ip = br_get_bridge_ip(br);
    if (br_ip != 0) {
        *data++ = GROUP_JOIN_IP_TAG;
        memcpy(data, &br_ip, sizeof(br_ip));
        data += sizeof(br_ip);
    }

    /* pad the rest of the packet */
    length = (data - skb_mac_header(skb));
    if (length < skb->len) {
        memset(data, GROUP_JOIN_PADDING, skb->len - length);
    }
					 ...
					 const unsigned char *addr)
{
	...
	struct net_bridge_port *p, *ret = NULL;

	/* preempt is already disabled */
                               ...
                               struct sk_buff *skb)
{
    ...
    const unsigned char *data = skb_mac_header(skb) + 2 * ETH_ALEN + 2;
    ...
    if (real_len >= (num_addrs * ETH_ALEN + 1 + sizeof(src_ip)) &&
        (data[num_addrs * ETH_ALEN] == GROUP_JOIN_IP_TAG)) {
        memcpy(&src_ip, &data[num_addrs * ETH_ALEN + 1], sizeof(src_ip));
    }
    ...
    for (ix = 0; ix < num_addrs; ix++) {
        ...
        if (0 == memcmp(igmp_ah_addr, maddr, ETH_ALEN) ||
            ...
            0 == memcmp(ipv6_mcast_pref, maddr, sizeof(ipv6_mcast_pref)))
            continue;

        /* figure out if this entry already exists in the table */
        hash = br_mac_hash(maddr);
	me = br->mcast_hash[hash];
        ...
        if (me) {
            ...
            if (mrxp) {
                ...
                for (mrxm = mrxp->rx_mac_list; mrxm != NULL; mrxm = mrxm->next)
                    if (0 == memcmp(eth_hdr(skb)->h_source,
                                    mrxm->addr, ETH_ALEN))
                        break;

                if (mrxm) {
                    /* Update IP and ageing timer */
                    mrxm->ip = src_ip;
        ...
        me_new = kmalloc(sizeof(*me_new), GFP_ATOMIC);
        if (!me_new)
            continue;
        ...
        if (mrxp_update) {
            ...
            mrxm_new = kmalloc(sizeof(*mrxm_new), GFP_ATOMIC);
            if (mrxm_new) {
                memcpy(mrxm_new->addr, eth_hdr(skb)->h_source, ETH_ALEN);
                mrxm_new->ageing_timer = jiffies;		
                mrxm_new->direct_dst = _get_direct_port(br, mrxm_new->addr);
#ifdef BR_DEBUG_DIRECT
                printk("mc: new: %p(%d) %p\n",
		       mrxp_update->dst, mrxp_update->dst ? mrxp_update->dst->port_no : -1,
		       mrxm_new->direct_dst);
#endif
                if (src_ip) {
                    mrxm_new->ip = src_ip;
     ...
     for (ix = 0; ix < BR_HASH_SIZE; ix++) {
         ...
         while (me != NULL) {
             ...
             if (num_deletes >= num_total) {
                 struct net_bridge_mcast_entry *tmp;
                             ...
                             int offset)
{
    ...
    /* copy_to_user() can't be used in atomic context. Fix later. */
    spin_lock_bh(&br->mcast_lock);
#endif
             ...
             if (bytes_needed <= buf_len) {
                 ...
                 for (mrxp = me->rx_port_list; 
                     ...
                     for (mrxm = mrxp->rx_mac_list; 
                         ...
                         /*
                          * Rollover is a nasty beast, so be careful here...
                          */
                         jiffies_used =
                             jiffies - mrxm->ageing_timer;

                         if (jiffies_used < br->mcast_ageing_time ) {
                             jiffies_left = br->mcast_ageing_time - jiffies_used;
                         }

                         secs_left = jiffies_left / HZ;
                    ...
                    struct net_bridge_port *p)
{
	 ...
	 * 3) DA of this device, PROTO of 0x6970 encapsulating #1 (some APs
	 *    convert broadcast/multicast to unicast).
	 *
	...
	if (ether->h_proto == htons(BR_MCAST_GL_PROTOCOLNUM)) {
		...
		if ((0 == memcmp(ether->h_dest, broadcast_addr, 6)) ||
		    (0 == memcmp(ether->h_dest, br->dev->dev_addr, 6))) {
...
void sonos_set_multicast_list(struct net_device *dev)
{
	...
	netdev_for_each_mc_addr(ha, dev) {
		...
		if (0 == memcmp(igmp_ah_addr,    ha->addr, 6) ||
		    ...
		    0 == memcmp(ipv6_mcast_pref, ha->addr, sizeof(ipv6_mcast_pref)))
			continue;
...
void br_stats_update(struct net_bridge *br,
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(5, 4, 0)
void br_mcast_timer_expired(unsigned long arg)
{
	struct net_bridge *br = (struct net_bridge *) arg;
#else
void br_mcast_timer_expired(struct timer_list *t)
{
	struct net_bridge *br = from_timer(br, t, mcast_timer);
#endif
...
void br_udp_overwrite_ip(struct sk_buff *skb, uint32_t dst_ip)
{
	...
	if (ethh->h_proto == htons(ETH_P_IP)) {
		...
		if (iph->protocol == IPPROTO_UDP) {
			__wsum csum;
			memcpy(&iph->daddr, &dst_ip, sizeof(iph->daddr));
			iph->check = 0;
...
static void get_bcmc_hit(struct __br_bcmc_hit *bcmc_stats,
                         struct net_bridge_bcmc_hit *bcmc_history)
{
	memcpy(bcmc_stats->src, bcmc_history->src, ETH_ALEN);
	memcpy(bcmc_stats->dest, bcmc_history->dest, ETH_ALEN);
	bcmc_stats->timestamp = bcmc_history->timestamp;
	bcmc_stats->packet_count = bcmc_history->packet_count;
	bcmc_stats->packet_type = bcmc_history->packet_type;
}
...
static void get_stats(struct net_bridge *br, struct __br_stats *stats)
{
	...
	for (i = 0; i < BR_BCMC_HIST_SIZE; i++) {
		get_bcmc_hit(&(stats->bcmc_history[i]), &(br->br_stats.bcmc_history[i]));
	}
}

int sonos_get_stats(struct net_bridge *br, void __user *userbuf)
{
	...
	if (copy_to_user(userbuf, &stats, sizeof(stats))) {
		ret = -EFAULT;
	}
	return ret;
}
