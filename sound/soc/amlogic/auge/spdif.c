...
static void spdifin_trigger_event(struct aml_spdif *p_spdif)
{

	extcon_set_state(p_spdif->edev,
		EXTCON_SPDIFIN_SAMPLERATE, 1);
	if (p_spdif->pc_changed) {
		p_spdif->pc_changed = 0;
		pr_info("Sample rate stable Pc changed\n");
		extcon_set_state(p_spdif->edev,
		...
		if (p_spdif->timer_counter > MAX_TIMER_COUNTER) {
			...
			if (p_spdif->is_reset == 0) {
				...
				if (p_spdif->sample_rate_detect_start == 1) {
					...
					mode = spdifin_get_sample_rate();
					if (p_spdif->last_event_mode != mode) {
						p_spdif->last_event_mode
							= mode;
						pr_info("state set mode=%d\n",
							mode);
						spdifin_trigger_event(p_spdif);
					}
					pr_debug(
					"%s,last sample mode:0x%x,stop timer\n",
					__func__,
...
static void spdifin_status_event(struct aml_spdif *p_spdif)
{
	...
	if (intrpt_status & 0x4) {
		...
		/*pr_info("state set samplerate 1\n");
				extcon_set_state(p_spdif->edev,
					EXTCON_SPDIFIN_SAMPLERATE, 1);
		*/
			}
