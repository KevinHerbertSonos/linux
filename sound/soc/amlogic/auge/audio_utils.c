				      ...
				      struct snd_ctl_elem_value *ucontrol)
{
	...
	for (pos = 0;
	     pos < (sizeof(ucontrol->value.iec958.status) / sizeof(int));
	     pos++) {
...
static int spdif_channel_status_get_mask(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
				       ...
				       struct snd_ctl_elem_value *ucontrol)
{
	...
	for (pos = 0;
		...
		audiobus_write(SPDIFOUT_CHSTS_REG(pos + 6),
			       ntohl(bitrev32(*reg)));
		reg++;
	}
...
static const struct snd_kcontrol_new snd_auge_controls[] = {
	...
	/*SPDIFOUT swap*/
	SND_SPDIFOUT_SWAP("SPDIFOUT Lane0 Left Channel Swap", SPDIFOUT,
			  out_swap_channel_enum, 0, 0x7),
	SND_SPDIFOUT_SWAP("SPDIFOUT Lane0 Right Channel Swap", SPDIFOUT,
			  out_swap_channel_enum, 4, 0x7),
	/*SPDIFOUT mixer*/
	SND_MIX("SPDIFOUT Mixer Channel", SPDIFOUT, lane0_mixer_enum, 23, 0x1),
	/* SPDIFOUT Channel Status */
	{
		.iface = SNDRV_CTL_ELEM_IFACE_MIXER,
		.name = "SPDIFOUT " SNDRV_CTL_NAME_IEC958("", PLAYBACK,
							  DEFAULT),
		.access = SNDRV_CTL_ELEM_ACCESS_READ |
	...
	/* audio locker */
	SOC_ENUM_EXT("audio locker enable", audio_locker_enum,
		     audio_locker_get_enum, audio_locker_set_enum),

	/* audio inskew */
	SOC_ENUM_EXT("audio inskew set", audio_inskew_enum,
		     audio_inskew_get_enum, audio_inskew_set_enum),
	/* tdmc out binv */
	SOC_ENUM_EXT("tdmout_c binv set", tdmout_c_binv_enum,
		     tdmout_c_binv_get_enum, tdmout_c_binv_set_enum),
};
