			       ...
			       struct asoc_simple_dai *simple_dai)
{
	...
	/* Get clock id if provided otherwise set to 0 */
	simple_dai->clk_id = 0;
	if (!of_property_read_u32(node, "clock-id", &val)) {
			      ...
			      struct asoc_simple_dai *simple_dai)
{
	...
	if (simple_dai->sysclk) {
		ret = snd_soc_dai_set_sysclk(dai, simple_dai->clk_id, simple_dai->sysclk, 0);
		if (ret && ret != -ENOTSUPP) {
