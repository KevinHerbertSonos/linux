...
static int switch_to_cpu_link_status = 0;
...
void mv88e6020_announce_linkup(struct net_device *dev)
{
	struct net *rtnl;
	struct sk_buff *skb;
	struct nlmsghdr *nlh;
...
static void mv88e6020_check_link(void)
{
	...
	switch_to_cpu_link_status = link;

	if (mv88e6020_phydev == NULL) {
		printk("mv88e6020_phydev NULL\n");
		return;
	}
	mv88e6020_announce_linkup(mv88e6020_phydev->attached_dev);
}
...
static void mv88e6020_link_change(struct work_struct *work)
{
	...
	while (gb2int != 0) {
		...
		gb2int = mdiobus_read(mv88e6020_phydev->bus,
				MV88E6020_GLOBAL2_REGS_ADDR,
...
static void mv88e6020_init_irq(struct phy_device *phydev)
{
	int ret;
        ...
        if (node) {
                mv88e6020_irq = gpio_to_irq(of_get_named_gpio(node, "interrupts", 0));
        }
...
#ifdef CONFIG_PM
static int m88e6020_suspend(struct phy_device *phydev)
{
        return 0;
}

static int m88e6020_resume(struct phy_device *phydev)
{
        return 0;
}
#endif
...
static int m88e6020_config_init(struct phy_device *phydev)
{
	...
	schedule_delayed_work(&mv88e6020_errata34_work, HZ);
	mdiobus_write(mv88e6020_phydev->bus, MV88E6020_PORT5_SWITCH_ADDR, 1, 0xC03D);
...
#ifdef CONFIG_PM
		.suspend       = &m88e6020_suspend,
		.resume        = &m88e6020_resume,
#endif
...
// stub it for now
int mv88e6020_phy_power_down(struct phy_device *phydev, int on)
{
    return 0;
}

subsys_initcall(mv88e6020_init);
