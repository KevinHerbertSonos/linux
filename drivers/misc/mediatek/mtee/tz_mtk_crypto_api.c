...
int mtk_crypto_driver_init(void)
{
	...
	ret = KREE_CreateSession(TZ_TA_SECURE_API_UUID,
							&secure_api_session);
...
)
{
	...
	ret = KREE_TeeServiceCall(secure_api_session,
				TZCMD_SECURE_API_CRYPTO_CTX_INIT,
	...
	pctx->mutex = (u64)(unsigned long)kmalloc(sizeof(struct mutex),
					GFP_KERNEL);
...
)
{
	...
	/* send command */
	ret = KREE_TeeServiceCall(secure_api_session,
				TZCMD_SECURE_API_CRYPTO_CTX_UNINIT,
...
)
{
	...
	/* send command */
	ret = KREE_TeeServiceCall(secure_api_session,
				TZCMD_SECURE_API_CRYPTO_RGN_GEN,
				TZ_ParamTypes2(TZPT_MEM_INPUT, TZPT_MEM_OUTPUT),
				param);
	if (ret != TZ_RESULT_SUCCESS) {
		if (copied)
...
u8 *tag
	...
	/* send command */
	ret = KREE_TeeServiceCall(secure_api_session,
			TZCMD_SECURE_API_CRYPTO_SET_AAD,
			TZ_ParamTypes3(TZPT_MEM_INOUT, TZPT_MEM_INPUT, TZPT_MEM_INOUT),
			param);
	if (ret != TZ_RESULT_SUCCESS) {
		pr_warn("KREE_TeeServiceCall fail, error:%d\n", ret);
		return -ERROR_MTK_CRYPTO_MTEE_OPERATION_FAIL;
...
const u8 *piv,	/* [in]  initial vector/counter buffer, 16 bytes */
				/*	== NULL for ecb mode */
				/*	!= NULL for cbc/ctr mode */
const u8 *pkey,	/* [in]  key buffer */
u32  key_len,	/* [in]  key length (byte unit) */
			/* 16 (128 bits), 24(192 bits), 32(256 bits) */
const u8 *psrc, /* [in]  source data buffer */
u8 *pdes,	 /* [out] destination data buffer, memory prepared by caller */
u32  dat_len	/* [in]  data buf length(byte unit), multiple of 16 */
)
{
	union MTEEC_PARAM param[4];
	int param_index = 0;
	...
	/* arrange params */
	packed_buffer =
		kmalloc(AES_IV_SIZE + sizeof(u8) * 2 + sizeof(struct mtk_crypto_ctx),
				GFP_KERNEL);
	if (packed_buffer == NULL)
		goto end;

	memset(packed_buffer,
		0,
		AES_IV_SIZE + sizeof(u8) * 2 + sizeof(struct mtk_crypto_ctx));
	...
	*(packed_buffer + AES_IV_SIZE + sizeof(u8)) = blk_mode;
	memcpy(packed_buffer + AES_IV_SIZE + sizeof(u8) * 2,
		pctx,
		sizeof(struct mtk_crypto_ctx));

	param[param_index].mem.buffer = (void *)packed_buffer;
	param[param_index].mem.size =
		AES_IV_SIZE + sizeof(u8) * 2 + sizeof(struct mtk_crypto_ctx);
	param_index++;
	...
	/* copy pctx back */
	memcpy(pctx,
		packed_buffer + AES_IV_SIZE + sizeof(u8) * 2,
		sizeof(struct mtk_crypto_ctx));
	result = 0;
...
struct mtk_crypto_ctx *pctx,	/* [in]  context of mtk crypto driver */
u8		  key_mode,	/* [in]  0 -> plain key */
					/* 1 -> key encrypted by hw key idx 0 */
					/* 2 -> key encrypted by hw key idx 1 */
u8        blk_mode,		/* [in]  0 -> ecb, 1 -> cbc, 2 -> ctr, 3 -> GCM */
u8        op_mode,		/* [in]  0 -> enc, 1 -> dec */
const u8 *piv,			/* [in]  initial vector or initial counter value buffer, 16 bytes */
					/*	== NULL for ecb mode */
...
)
{
	...
	/* send command */
	ret = KREE_TeeServiceCall(secure_api_session,
			TZCMD_SECURE_API_CRYPTO_SHA256_BEGIN,
...
)
{
	...
	if (pctx->buf_idx > 0 ||
		(dat_len & SHA256_BLOCK_SIZE_MASK) ||
		!isKernelLowmem(pdat)) {
		...
		if (wk_buf_block_count > 0) {
			...
			/* send command */
			ret = KREE_TeeServiceCall(secure_api_session,
					TZCMD_SECURE_API_CRYPTO_SHA256_PROCESS,
...
struct mtk_crypto_ctx *pctx, /* [in]  context of mtk crypto driver */
u8 *pres	/* [out] hash res buffer, 32 bytes, buffer prepared by caller */
)
{
	union MTEEC_PARAM param[4];
	int ret;
	...
	/* send command */
	ret = KREE_TeeServiceCall(secure_api_session,
			TZCMD_SECURE_API_CRYPTO_SHA256_DONE,
			TZ_ParamTypes2(TZPT_MEM_INPUT, TZPT_MEM_OUTPUT),
			param);
	if (ret != TZ_RESULT_SUCCESS) {
		pr_warn("KREE_TeeServiceCall fail, ret:%d\n", ret);
...
)
{
	...
	/* send command */
	ret = KREE_TeeServiceCall(secure_api_session,
			TZCMD_SECURE_API_CRYPTO_SHA256,
