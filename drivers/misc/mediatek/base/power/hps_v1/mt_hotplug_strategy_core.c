...
static int hps_enable_back = 0;

static int hps_suspend(struct platform_device *dev, pm_message_t state)
{
	log_info("%s\n", __func__);

	mutex_lock(&hps_ctxt.lock);
	hps_enable_back = hps_ctxt.enabled;
	hps_ctxt.enabled = 0;
	mutex_unlock(&hps_ctxt.lock);
...
static int hps_resume(struct platform_device *dev)
{
	log_info("%s\n", __func__);

	mutex_lock(&hps_ctxt.lock);
	hps_ctxt.enabled = hps_enable_back;
	mutex_unlock(&hps_ctxt.lock);
...
static struct platform_driver hotplug_strategy_pdrv = {
	.suspend = hps_suspend,
	.resume = hps_resume,
	.driver = {
		.name = "hps",
...
int hps_core_init(void)
{
	...
	if (r)
		hps_err("platform_device_register fail(%d)\n", r);

	r = platform_driver_register(&hotplug_strategy_pdrv);
	if (r)
