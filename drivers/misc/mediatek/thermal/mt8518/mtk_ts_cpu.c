...
static int trip_temp[10] = {105000, 92000, 85000, 75000, 65000,
...
static int num_trip = 1; /* Sonos uses 1 trip */
...
#define NORMAL_TEMP_POLL (250)
static int temp_update_interval = 250;
...
irqreturn_t thermal_interrupt_handler(int irq, void *dev_id)
{
	u32 ret = 0;
	unsigned long flags;
...
struct delayed_work reboot_delay_queue;
static unsigned int reboot_delay_queue_status = 0;
static void reboot_delay_work(struct work_struct *work)
{
	pr_info("\n\n************ Too Hot  ************\n");
	kernel_restart(NULL);
}
...
static int sysrst_cpu_set_cur_state(struct thermal_cooling_device *cdev,
				    unsigned long state)
{
	static struct workqueue_struct *reboot_delay_wq;
	cl_dev_sysrst_state = state;

	if (cl_dev_sysrst_state && reboot_delay_queue_status == 0 ) {
		mtkts_dump_cali_info();
		...
		reboot_delay_wq = create_workqueue("reboot_delay_wq");
		if (!reboot_delay_wq) {
			printk(KERN_ERR "No memory for workqueue\n");
			return 1;
		}

		INIT_DELAYED_WORK(&reboot_delay_queue, reboot_delay_work);
		queue_delayed_work_on(0, reboot_delay_wq,
				&reboot_delay_queue,
				round_jiffies(msecs_to_jiffies(12000)));
		reboot_delay_queue_status = 1;
	}
	return 0;
}
...
static enum hrtimer_restart tscpu_update_tempinfo(struct hrtimer *timer)
{
	...
	ktime = ktime_set(0, temp_update_interval * 1000000); /* ms */
