...
int fsl_lpspi_bytes_per_word(const int bpw)
{
	return DIV_ROUND_UP(bpw, BITS_PER_BYTE);
}

bool fsl_lpspi_can_dma(struct spi_master *controller,
...
static int lpspi_prepare_xfer_hardware(struct spi_master *controller)
{
...
static int lpspi_unprepare_xfer_hardware(struct spi_master *controller)
{
	struct fsl_lpspi_data *fsl_lpspi =
				spi_master_get_devdata(controller);
...
static int fsl_lpspi_dma_configure(struct spi_master *controller)
{
	int ret;
...
static int fsl_lpspi_setup_transfer(struct spi_master *controller,
				     struct spi_device *spi,
...
int fsl_lpspi_slave_abort(struct spi_master *controller)
{
	struct fsl_lpspi_data *fsl_lpspi =
				spi_master_get_devdata(controller);
...
static int fsl_lpspi_wait_for_completion(struct spi_master *controller)
{
	struct fsl_lpspi_data *fsl_lpspi =
				spi_master_get_devdata(controller);
...
static int fsl_lpspi_dma_transfer(struct spi_master *controller,
				struct fsl_lpspi_data *fsl_lpspi,
				struct spi_transfer *transfer)
{
...
static void fsl_lpspi_dma_exit(struct spi_master *controller)
{
	if (controller->dma_rx) {
...
static int fsl_lpspi_pio_transfer(struct spi_master *controller,
				  struct spi_transfer *t)
{
	struct fsl_lpspi_data *fsl_lpspi =
				spi_master_get_devdata(controller);
...
static int fsl_lpspi_transfer_one(struct spi_master *controller,
				  struct spi_device *spi,
				  struct spi_transfer *t)
{
	struct fsl_lpspi_data *fsl_lpspi =
...
int fsl_lpspi_spi_transfer(struct spi_device *spi, struct spi_message *msg)
{
	return 0;
}
...
static int fsl_lpspi_probe(struct platform_device *pdev)
{
	...
	if (!fsl_lpspi->is_slave) {
		...
		master->cs_gpios = fsl_lpspi->chipselect;
		master->prepare_message = fsl_lpspi_prepare_message;
	}

	master->transfer_one = fsl_lpspi_transfer_one;
	...
	master->setup = fsl_lpspi_setup;
	master->cleanup = fsl_lpspi_cleanup;
	master->bus_num = pdev->id;
	...
	ret = fsl_lpspi_dma_init(&pdev->dev, fsl_lpspi, master);
	if (ret == -EPROBE_DEFER)
		goto out_master_put;
