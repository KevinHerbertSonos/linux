...
int btmtk_sdio_notify_wlan_remove_start(void)
{
	...
	int ret = 0;
	typedef int (*pnotify_wlan_remove_start) (int reserved);
	char *notify_wlan_remove_start_func_name = "BT_rst_L0_notify_WF_step1";
...
static int btmtk_sdio_wait_dump_complete_thread(void *ptr)
{
	...
	if (i >= 60)
		pr_warn("%s: wait dump complete timeout\n", __func__);
		...
		char *searchcontent)
{
	...
	if (text == NULL) {
		pr_err("%s: text param is invalid!\n", __func__);
		ret = false;
		goto out;
	}
		...
		char *searchcontent)
{
	...
	if (ret) {
		...
		pr_warn("%s: search item %s is invalid!\n", __func__, BT_UNIFY_WOBLE);
	}
...
static int btmtk_sdio_load_woble_block_setting(char *block_name,
...
static void btmtk_sdio_load_woble_setting_callback(const struct firmware *fw_data, void *context)
{
	...
	if (!fw_data) {
		...
		if (need_retry_load_woble < BTMTK_LOAD_WOBLE_RETRY_COUNT) {
			...
			if (err < 0) {
				pr_err("%s: request %s file fail(%d)\n",
					__func__, g_card->woble_setting_file_name, err);
			}
		} else {
			pr_err("%s: request %s file fail(%d), need_load_origin_woble = %d\n",
		...
		struct btmtk_sdio_card *data, u32 *code_len)
{
	...
	*code_len = 0;
	...
	if (fw_entry) {
		...
		pr_err("%s: fw_entry is NULL request_firmware may fail!! error code = %d\n", __func__, err);
		return err;
	}

	if (data->setting_file == NULL) {
		pr_err("%s: kzalloc size %zu failed!\n!", __func__, fw_entry->size);
		...
		struct btmtk_sdio_card *data)
{
	...
	if (ptr_name) {
		...
		if (err != 0) {
			pr_err("%s, btmtk_sdio_load_code_from_cfg_files failed!!\n", __func__);
			return err;
		}

		if (!btmtk_sdio_load_bt_cfg_item(&data->bt_cfg, data->setting_file)) {
			pr_err("%s, btmtk_sdio_load_bt_cfg_item error!!\n", __func__);
			err = -1;
			return err;
		}
	} else
		pr_warn("%s: bin_name is not defined\n", __func__);
...
static u32 LOCK_UNSLEEPABLE_LOCK(struct _OSAL_UNSLEEPABLE_LOCK_ *pUSL)
{
	spin_lock_irqsave(&(pUSL->lock), pUSL->flag);
	return 0;
}

static u32 UNLOCK_UNSLEEPABLE_LOCK(struct _OSAL_UNSLEEPABLE_LOCK_ *pUSL)
{
...
static void btmtk_sdio_set_no_fw_own(struct btmtk_private *priv, bool no_fw_own)
{
	if (priv) {
		...
		printk_ratelimited(KERN_DEBUG" set no_fw_own %d\n", priv->no_fw_own);
	} else
		printk_ratelimited(KERN_DEBUG" priv is NULL\n");
}
...
static int btmtk_efuse_read(u16 addr, u8 *value)
{
	...
	if (ret) {
		pr_warn("%s: btmtk_sdio_send_hci_cmd error\n", __func__);
...
void btmtk_set_keep_full_pwr(uint8_t *buf, size_t buf_size)
{
	...
	if (*ptr == PWR_KEEP_NO_FW_OWN) {
		...
		pr_warn("%s: It's not the correct own setting: %c\n", __func__, *ptr);
	}
...
static int btmtk_sdio_set_audio(void)
{
	...
	if (ret) {
		pr_err("btmtk_sdio_set_audio_slave error(%d)\n", ret);
		return ret;
	}

	ret = btmtk_sdio_set_audio_pin_mux();
	if (ret) {
		pr_err("btmtk_sdio_set_audio_pin_mux error(%d)\n", ret);
		return ret;
	}
		...
		int add_spec_header)
{
	...
	if (rx_length > (SDIO_HEADER_LEN + 8) && rxbuf[SDIO_HEADER_LEN] == 0x80 &&
		...
		if (dump_data_counter % 1000 == 0)
			pr_warn("%s: coredump on-going, total_packet = %d, total_length = %d\n",
					__func__,
		...
		if (skb_queue_len(&g_priv->adapter->fwlog_fops_queue) < FWLOG_ASSERT_QUEUE_COUNT) {
			...
			pr_warn("btmtk_sdio FW dump queue size is full");
		}
		...
		if (dump_len >= strlen(FW_DUMP_END_EVENT)) {
			...
			if (core_dump_end) {
				pr_warn("%s: core_dump_end %s, total_packet = %d, total_length = %d, rxbuf = %02x %02x %02x\n",
		...
		pr_err("%s: This is btclk event, status:%02x\n",
		...
		if (rxbuf[SDIO_HEADER_LEN + 12] == 0x0) {
			...
			pr_warn("%s: No ACL CONNECTION(%d), disable event and interrupt\n",
...
static int btmtk_sdio_push_data_to_metabuffer(struct ring_buffer *metabuffer,
		char *data, int len, u8 type, bool use_type)
{
	int remainLen = 0;
...
static int btmtk_sdio_pull_data_from_metabuffer(struct ring_buffer *metabuffer,
...
{
	struct sk_buff *skb = NULL;
	u8 coredump_cmd[] = {0x6F, 0xFC, 0x05, 0x00,
		...
		const struct sdio_device_id *id)
{
	...
	if (func->num != data->reg->func_num) {
		pr_err(L0_RESET_TAG "func num is not match\n");
		return -ENODEV;
		}
	...
	if (btmtk_sdio_register_dev(card) < 0) {
		pr_err(L0_RESET_TAG "Failed to register BT device!\n");
		return -ENODEV;
	}
	...
	if (btmtk_sdio_download_fw(card)) {
		pr_err(L0_RESET_TAG "Downloading firmware failed!\n");
		fw_download_fail = 1;
	}
...
unreg_dev:
	...
	pr_err(L0_RESET_TAG "%s fail end\n", __func__);
	return ret;
}
#endif
...
static int btmtk_sdio_L0_reset_host_config(struct mmc_host *host)
{
	...
	if (host == NULL) {
		pr_err(L0_RESET_TAG "%s mmc host is NULL\n", __func__);
		return -1;
	}
...
static int btmtk_sdio_L0_reset(struct mmc_card *card)
{
	...
	if ((card == NULL) || (card->host  == NULL)) {
		pr_err(L0_RESET_TAG "%s mmc structs are NULL\n", __func__);
		return ret;
	}
	...
	if (ret != 0) {
		pr_err(L0_RESET_TAG "%s set SDIO host failed\n", __func__);
		return ret;
	}
...
int btmtk_sdio_reset_dongle(void)
{
	...
	if ((!g_card) || (!g_card->func) || (!g_card->func->card)) {
		pr_err(L0_RESET_TAG "data corrupted\n");
		goto rst_dongle_done;
	}
...
static int btmtk_sdio_trigger_fw_assert(void)
{
	...
	u8 cmd_7663[] = { 0x6F, 0xFC, 0x05, 0x01, 0x02, 0x01, 0x00, 0x08 };
...
static int btmtk_sdio_handle_leaving_WoBLE_state(void)
{
	...
	if (g_card->woble_setting_radio_on[0].length &&
		...
		if (ret) {
			pr_err("%s: woble radio on error", __func__);
			goto finish;
		}
...
static int btmtk_sdio_suspend(struct device *dev)
{
	...
	if (func) {
		...
		if (!(pm_flags & MMC_PM_KEEP_POWER)) {
			pr_err("%s:%s cannot remain alive while suspended(0x%x)\n",
...
{
	if (pFn_Probe == NULL) {
		pr_err(L0_RESET_TAG "%s new probe is NULL", __func__);
		return;
	}
...
static int btmtk_fops_open(struct inode *inode, struct file *file)
{
	...
	pr_err("%s: Mediatek Bluetooth SDIO driver ver %s\n", __func__, VERSION);
...
static ssize_t btmtk_fops_writefwlog(struct file *filp,
