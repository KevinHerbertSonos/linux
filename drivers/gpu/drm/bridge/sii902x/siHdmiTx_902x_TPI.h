...
struct SIHDMITX_CONFIG {
	...
	//010:MCLK=384Fs; 011:MCLK=512Fs;
	//100:MCLK=768Fs; 101:MCLK=1024Fs;
	//110:MCLK=1152Fs; 111:MCLK=192Fs;
	//[3]_WS Polarity-Left when:
	//0 = WS is low when Left; 1 = WS is high when Left
...
#define IsCEC_DEVICE()	\
	(((g_edid.CEC_A_B != 0xFF) &&	\
	(g_edid.CEC_C_D != 0xFF)) ? TRUE : FALSE)
...
#define TPI_TRACE_PRINT(x)	(pr_info x)
#else
...
#define TPI_DEBUG_PRINT(x)	(pr_info x)
#else
#define TPI_DEBUG_PRINT(x)
#endif

// EDID Print Macro
// Note: To enable EDID description printing,
//both CONF__TPI_EDID_PRINT and
//CONF__TPI_DEBUG_PRINT must be enabled
...
// Note: To enable CPI description printing,
//both CONF__CPI_DEBUG_PRINT and
//CONF__TPI_DEBUG_PRINT must be enabled
...
#define ClearInterrupt(x)  \
	WriteByteTPI(TPI_INTERRUPT_STATUS_REG, x)
...
#define EN_AND_RPT_AVI				0xC0
#define DISABLE_AVI					0x00
...
#define TPI_AVI_INFO_REG_ADDR				(0x0C)
