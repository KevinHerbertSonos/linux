...
int sonos_sar_adc_iio_info_read_raw(int chan, int *mvolts)
{  // move to meson_saradn)
	const struct iio_chan_spec *ad_chan = indio_dev->channels + chan;

	meson_sar_adc_get_sample(indio_dev, ad_chan, NO_FILTER, ONE_SAMPLE,
				 mvolts);
	return 0;
}
