...
static ssize_t show_boot_kpub_sha(struct class *cla, struct class_attribute *attr,char *buf)
{
	int ret = -1;
	unsigned char buffer[32];
	size_t len = sizeof(buffer);
	ret = efuse_obj_read(EFUSE_OBJ_SBOOT_KPUB_SHA, buffer, len);
	if (ret > 0)
...
static ssize_t store_boot_kpub_sha(struct class *cla,
	struct class_attribute *attr, const char *buf, size_t count)
{
	int ret = -1;
	if (count < 32)
		return 1;
	ret = efuse_obj_write(EFUSE_OBJ_SBOOT_KPUB_SHA, (char *)buf, count);
	if (ret < 0)
		pr_err("failed to efuse_obj_write boot_kpub_sha\n");
	return count;
}

static ssize_t show_antirollback(struct class *cla, struct class_attribute *attr,char *buf)
{
	int ret = -1;
	unsigned char buffer[32];
	size_t len = sizeof(buffer);
	ret = efuse_obj_read(EFUSE_OBJ_LICENSE_ENABLE_ANTIROLLBACK, buffer, len);
	pr_info("%s buffer:%d, ret:%d \n", __func__, buffer[0], ret);
	if (ret > 0)
...
static ssize_t store_antirollback(struct class *cla,
	struct class_attribute *attr, const char *buf, size_t count)
{
	int ret = -1;
	ret = efuse_obj_write(EFUSE_OBJ_LICENSE_ENABLE_ANTIROLLBACK, NULL, 0);
	if (ret < 0)
		pr_err("failed to efuse_obj_write antirollback\n");
	return count;
}

static ssize_t show_boot_encrypt(struct class *cla, struct class_attribute *attr,char *buf)
{
	int ret = -1;
	unsigned char buffer[32];
	size_t len = sizeof(buffer);
	ret = efuse_obj_read(EFUSE_OBJ_LICENSE_ENABLE_ENCRYPTION, buffer, len);
	if (ret > 0)
		memcpy(buf, buffer, ret);
	else
		buf[0] = 0;
...
static ssize_t store_boot_encrypt(struct class *cla,
	struct class_attribute *attr, const char *buf, size_t count)
{
	int ret = -1;
	ret = efuse_obj_write(EFUSE_OBJ_LICENSE_ENABLE_ENCRYPTION, NULL, 0);
	if (ret < 0)
		pr_err("failed to efuse_obj_write boot_encrypt\n");
	return count;
}

static ssize_t show_boot_verity(struct class *cla, struct class_attribute *attr,char *buf)
{
	int ret = -1;
	unsigned char buffer[32];
	size_t len = sizeof(buffer);
	ret = efuse_obj_read(EFUSE_OBJ_LICENSE_ENABLE_SECURE_BOOT, buffer, len);
	if (ret > 0)
		memcpy(buf, buffer, ret);
	else
		buf[0] = 0;
...
static ssize_t store_boot_verity(struct class *cla,
	struct class_attribute *attr, const char *buf, size_t count)
{
	int ret = -1;
	ret = efuse_obj_write(EFUSE_OBJ_LICENSE_ENABLE_SECURE_BOOT, NULL, 0);
	if (ret < 0)
		pr_err("failed to efuse_obj_write boot_verity\n");
	return count;
}

static ssize_t show_jtag_disable(struct class *cla, struct class_attribute *attr,char *buf)
{
	int ret = -1;
	unsigned char buffer[32];
	size_t len = sizeof(buffer);
	ret = efuse_obj_read(EFUSE_OBJ_LICENSE_DISABLE_JTAG, buffer, len);
	if (ret > 0)
		memcpy(buf, buffer, ret);
	else
		buf[0] = 0;
...
static ssize_t store_jtag_disable(struct class *cla,
	struct class_attribute *attr, const char *buf, size_t count)
{
	int ret = -1;
	ret = efuse_obj_write(EFUSE_OBJ_LICENSE_DISABLE_JTAG, NULL, 0);
	if (ret < 0)
		pr_err("failed to efuse_obj_write jtag_disable\n");
	return count;
}

static ssize_t show_jtag_protect(struct class *cla, struct class_attribute *attr,char *buf)
{
	int ret = -1;
	unsigned char buffer[32];
	size_t len = sizeof(buffer);
	ret = efuse_obj_read(EFUSE_OBJ_LICENSE_ENABLE_JTAG_PASSWORD, buffer, len);
	if (ret > 0)
		memcpy(buf, buffer, ret);
	else
		buf[0] = 0;
...
static ssize_t store_jtag_protect(struct class *cla,
	struct class_attribute *attr, const char *buf, size_t count)
{
	int ret = -1;
	ret = efuse_obj_write(EFUSE_OBJ_LICENSE_ENABLE_JTAG_PASSWORD, NULL, 0);
	if (ret < 0)
		pr_err("failed to efuse_obj_write jtag_protect\n");
	return count;
}

static ssize_t show_scan_disable(struct class *cla, struct class_attribute *attr,char *buf)
{
	int ret = -1;
	unsigned char buffer[32];
	size_t len = sizeof(buffer);
	ret = efuse_obj_read(EFUSE_OBJ_LICENSE_DISABLE_SCAN, buffer, len);
	if (ret > 0)
		memcpy(buf, buffer, ret);
	else
		buf[0] = 0;
...
static ssize_t store_scan_disable(struct class *cla,
	struct class_attribute *attr, const char *buf, size_t count)
{
	int ret = -1;
	ret = efuse_obj_write(EFUSE_OBJ_LICENSE_DISABLE_SCAN, NULL, 0);
	if (ret < 0)
		pr_err("failed to efuse_obj_write scan_disable\n");
	return count;
}

static ssize_t show_scan_protect(struct class *cla, struct class_attribute *attr,char *buf)
{
	int ret = -1;
	unsigned char buffer[32];
	size_t len = sizeof(buffer);
	ret = efuse_obj_read(EFUSE_OBJ_LICENSE_ENABLE_SCAN_PASSWORD, buffer, len);
	if (ret > 0)
		memcpy(buf, buffer, ret);
	else
		buf[0] = 0;
...
static ssize_t store_scan_protect(struct class *cla,
	struct class_attribute *attr, const char *buf, size_t count)
{
	int ret = -1;

	ret = efuse_obj_write(EFUSE_OBJ_LICENSE_ENABLE_SCAN_PASSWORD, NULL, 0);
	if (ret < 0)
		pr_err("failed to efuse_obj_write scan_protect\n");
	return count;
}

static ssize_t show_usb_boot_disable(struct class *cla, struct class_attribute *attr,char *buf)
{
	int ret = -1;
	unsigned char buffer[32];
	size_t len = sizeof(buffer);

	ret = efuse_obj_read(EFUSE_OBJ_LICENSE_DISABLE_USB_BOOT, buffer, len);
	if (ret > 0)
		memcpy(buf, buffer, ret);
	else
		buf[0] = 0;
...
static ssize_t store_usb_boot_disable(struct class *cla,
	struct class_attribute *attr, const char *buf, size_t count)
{
	int ret = -1;

	ret = efuse_obj_write(EFUSE_OBJ_LICENSE_DISABLE_USB_BOOT, NULL, 0);
	if (ret < 0)
		pr_err("failed to efuse_obj_write usb_boot_disable\n");
	return count;
}

static ssize_t show_usb_boot_protect(struct class *cla, struct class_attribute *attr,char *buf)
{
	int ret = -1;
	unsigned char buffer[32];
	size_t len = sizeof(buffer);

	ret = efuse_obj_read(EFUSE_OBJ_LICENSE_ENABLE_USB_BOOT_PASSWORD, buffer, len);
	if (ret > 0)
		memcpy(buf, buffer, ret);
	else
		buf[0] = 0;
...
static ssize_t store_usb_boot_protect(struct class *cla,
	struct class_attribute *attr, const char *buf, size_t count)
{
	int ret = -1;

	ret = efuse_obj_write(EFUSE_OBJ_LICENSE_ENABLE_USB_BOOT_PASSWORD, NULL, 0);
	if (ret < 0)
		pr_err("failed to efuse_obj_write usb_boot_protect\n");
	return count;
}

static ssize_t show_boot_revoke_kpub0(struct class *cla,
		struct class_attribute *attr, char *buf)
{
	int ret = -1;
	unsigned char buffer[32];
	...
	ret = efuse_obj_read(EFUSE_OBJ_LICENSE_REVOKE_KPUB_0, buffer, len);
	if (ret > 0)
		memcpy(buf, buffer, ret);
	else
		buf[0] = 0;
...
static ssize_t store_boot_revoke_kpub0(struct class *cla,
		struct class_attribute *attr, const char *buf, size_t count)
{
	int ret = -1;

	ret = efuse_obj_write(EFUSE_OBJ_LICENSE_REVOKE_KPUB_0, NULL, 0);
	if (ret < 0)
		pr_err("failed to efuse_obj_write boot_revoke_kpub0\n");
	return count;
}
static ssize_t show_boot_revoke_kpub1(struct class *cla,
		struct class_attribute *attr, char *buf)
{
	int ret = -1;
	...
	ret = efuse_obj_read(EFUSE_OBJ_LICENSE_REVOKE_KPUB_1, buffer, len);
	if (ret > 0)
		memcpy(buf, buffer, ret);
	else
		buf[0] = 0;
...
static ssize_t store_boot_revoke_kpub1(struct class *cla,
		struct class_attribute *attr, const char *buf, size_t count)
{
	int ret = -1;

	ret = efuse_obj_write(EFUSE_OBJ_LICENSE_REVOKE_KPUB_1, NULL, 0);
	if (ret < 0)
		pr_err("failed to efuse_obj_write boot_revoke_kpub1\n");
	return count;
}
static ssize_t show_boot_revoke_kpub2(struct class *cla,
		struct class_attribute *attr, char *buf)
{
	int ret = -1;
	unsigned char buffer[32];
	...
	ret = efuse_obj_read(EFUSE_OBJ_LICENSE_REVOKE_KPUB_2, buffer, len);
	if (ret > 0)
		memcpy(buf, buffer, ret);
	else
		buf[0] = 0;
...
static ssize_t store_boot_revoke_kpub2(struct class *cla,
		struct class_attribute *attr, const char *buf, size_t count)
{
	int ret = -1;

	ret = efuse_obj_write(EFUSE_OBJ_LICENSE_REVOKE_KPUB_2, NULL, 0);
	if (ret < 0)
		pr_err("failed to efuse_obj_write boot_revoke_kpub2\n");
	return count;
}
static ssize_t show_boot_revoke_kpub3(struct class *cla,
		struct class_attribute *attr, char *buf)
{
	int ret = -1;
	unsigned char buffer[32];
	...
	ret = efuse_obj_read(EFUSE_OBJ_LICENSE_REVOKE_KPUB_3, buffer, len);
	if (ret > 0)
		memcpy(buf, buffer, ret);
	else
		buf[0] = 0;
...
static ssize_t store_boot_revoke_kpub3(struct class *cla,
		struct class_attribute *attr, const char *buf, size_t count)
{
	int ret = -1;

	ret = efuse_obj_write(EFUSE_OBJ_LICENSE_REVOKE_KPUB_3, NULL, 0);
	if (ret < 0)
		pr_err("failed to efuse_obj_write boot_revoke_kpub3\n");
	return count;
}
static ssize_t show_boot_aes(struct class *cla,
		struct class_attribute *attr, char *buf)
{
	int ret = -1;
	unsigned char buffer[32];
	...
	ret = efuse_obj_read(EFUSE_OBJ_SBOOT_AES256_SHA2, buffer, len);
	if (ret > 0)
		memcpy(buf, buffer, ret);
	else
		buf[0] = 0;
...
static ssize_t store_boot_aes(struct class *cla,
		struct class_attribute *attr, const char *buf, size_t count)
{
	int ret = -1;

	ret = efuse_obj_write(EFUSE_OBJ_SBOOT_AES256_RAW, NULL, 0);
	if (ret < 0)
		pr_err("failed to efuse_obj_write boot_aes\n");
	return count;
}
static ssize_t show_jtag_scan_password(struct class *cla,
		struct class_attribute *attr, char *buf)
{
	int ret = -1;
	unsigned char buffer[32];
	...
	ret = efuse_obj_read(EFUSE_OBJ_JTAG_PASSWD_SHA_SALT, buffer, len);
	if (ret > 0)
		memcpy(buf, buffer, ret);
	else
		buf[0] = 0;
...
static ssize_t store_jtag_scan_password(struct class *cla,
		struct class_attribute *attr, const char *buf, size_t count)
{
	int ret = -1;

	ret = efuse_obj_write(EFUSE_OBJ_JTAG_PASSWD_SHA_SALT, NULL, 0);
	if (ret < 0)
		pr_err("failed to efuse_obj_write jtag_scan_pswd\n");
	return count;
}

/*General purpose REE OTP block*/
static ssize_t show_gp_otp_ree(struct class *cla,
		struct class_attribute *attr, char *buf)
{
	int ret = -1;
	...
	ret = efuse_obj_read(EFUSE_OBJ_GP_REE, buffer, len);
	if (ret > 0)
		memcpy(buf, buffer, ret);
	else
		buf[0] = 0;
...
static ssize_t store_gp_otp_ree(struct class *cla,
		struct class_attribute *attr, const char *buf, size_t count)
{
	int ret = -1;

	ret = efuse_obj_write(EFUSE_OBJ_GP_REE, NULL, 0);
	if (ret < 0)
		pr_err("failed to efuse_obj_write gp_otp_ree\n");
	return count;
}
#endif
...
#ifdef CONFIG_EFUSE_OBJ_API
	__ATTR(boot_kpub_sha, 0700, show_boot_kpub_sha, store_boot_kpub_sha),
	__ATTR(antirollback,  0700, show_antirollback, store_antirollback),
	__ATTR(boot_encrypt,  0700, show_boot_encrypt, store_boot_encrypt),
	__ATTR(boot_verify,    0700, show_boot_verity, store_boot_verity),
	__ATTR(jtag_disable,   0700, show_jtag_disable, store_jtag_disable),
	__ATTR(jtag_protect,   0700, show_jtag_protect, store_jtag_protect),
	__ATTR(scan_disable,   0700, show_scan_disable, store_scan_disable),
	__ATTR(scan_protect,   0700, show_scan_protect, store_scan_protect),
	__ATTR(usb_boot_disable, 0700, show_usb_boot_disable,
			store_usb_boot_disable),
	__ATTR(usb_boot_protect, 0700, show_usb_boot_protect,
			store_usb_boot_protect),
	__ATTR(boot_revoke_kpub0, 0700, show_boot_revoke_kpub0,
			store_boot_revoke_kpub0),
	__ATTR(boot_revoke_kpub1, 0700, show_boot_revoke_kpub1,
			store_boot_revoke_kpub1),
	__ATTR(boot_revoke_kpub2, 0700, show_boot_revoke_kpub2,
			store_boot_revoke_kpub2),
	__ATTR(boot_revoke_kpub3, 0700, show_boot_revoke_kpub3,
			store_boot_revoke_kpub3),
	__ATTR(boot_aes, 0700, show_boot_aes, store_boot_aes),
	__ATTR(jtag_scan_password, 0700, show_jtag_scan_password,
			store_jtag_scan_password),
	__ATTR(gp_otp_ree, 0700, show_gp_otp_ree, store_gp_otp_ree),
