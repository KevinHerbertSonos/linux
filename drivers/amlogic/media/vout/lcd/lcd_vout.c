...
static int lcd_config_probe(struct platform_device *pdev)
{
	...
	/* select pinctrl */
	if (of_get_property(lcd_driver->dev->of_node, "pinctrl-names", NULL)) {
		clk12_pinctl = devm_pinctrl_get_select(
				lcd_driver->dev, "clk12");
		if (clk12_pinctl == NULL)
			pr_err("%s failed to pinctrl_get_select clk12\n",
					__func__);
	}
