...
void lcd_clk_gate_switch(int status)
{
	...
	if (status) {
		...
		case LCD_CHIP_AXG:
			...
			if (IS_ERR(clk12_enable_gate))
				LCDERR("%s: clk12_enable_gate\n", __func__);
			else
				clk_prepare_enable(clk12_enable_gate);
			break;
		case LCD_CHIP_G12A:
			if (IS_ERR(lcd_drv->dsi_host_gate))
				LCDERR("%s: dsi_host_gate\n", __func__);
...
static void lcd_clktree_probe(void)
{
	...
	case LCD_CHIP_AXG:
		...
		clk12_enable_gate = devm_clk_get(
			lcd_drv->dev, "clk12_enable_gate");
		if (IS_ERR(clk12_enable_gate))
			LCDERR("%s: clk clk12_enable_gate\n", __func__);
		break;
	case LCD_CHIP_G12A:
		lcd_drv->dsi_host_gate = devm_clk_get(lcd_drv->dev,
			"dsi_host_gate");
