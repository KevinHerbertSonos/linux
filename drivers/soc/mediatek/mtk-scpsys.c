...
static int scpsys_power_on(struct generic_pm_domain *genpd)
{
	...
	if (!strcmp(genpd->name, "audafe"))
		clk_disable_unprepare(scpd->clk[0]);
#endif
...
#if defined(CONFIG_MACH_MT8518)
	if (!strcmp(genpd->name, "audafe")) {
		ret = clk_prepare_enable(scpd->clk[0]);
		if (ret)
			goto out;
	}
#endif
...
#define NUM_DOMAINS_MT8518	ARRAY_SIZE(scp_domain_data_mt8518)

static int __init scpsys_probe_mt8518(struct platform_device *pdev)
{
	struct scp *scp;

	scp = init_scp(pdev, scp_domain_data_mt8518, NUM_DOMAINS_MT8518);
	if (IS_ERR(scp))
		return PTR_ERR(scp);

	mtk_register_power_domains(pdev, scp, NUM_DOMAINS_MT8518);
