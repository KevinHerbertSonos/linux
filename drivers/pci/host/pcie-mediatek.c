...
static int mtk_pcie_startup_port_v2(struct mtk_pcie_port *port)
{
	...
	if (gpio_is_valid(port->pcie_2g_reset)) {
        	gpio_set_value_cansleep(port->pcie_2g_reset, 1);
		gpio_free(port->pcie_2g_reset);
	}

	if (gpio_is_valid(port->pcie_5g_reset)) {
        	gpio_set_value_cansleep(port->pcie_5g_reset, 1);
		gpio_free(port->pcie_5g_reset);
	}
...
static int mtk_pcie_enable_port(struct mtk_pcie_port *port)
{
	...
	if(IS_ENABLED(CONFIG_PCIE_MEDIATEK_COMPLIANCE_TEST))
		return 1;
			       ...
			       int slot)
{
	...
	err = of_property_read_u32(node, "speed", &port->speed);
	if (err) {
		pr_info("missing speed property\n");
		return err;
	}
	...
	port->pcie_2g_reset = of_get_named_gpio(node, "rst-2g-gpio", 0);
	if (gpio_is_valid(port->pcie_2g_reset)) {
		int val;
		val = gpio_request_one(port->pcie_2g_reset,
				GPIOF_OUT_INIT_LOW, "WIFI Reset");
		if ( val ) {
			pr_err("Could not request %d enable gpio: %d\n",
				port->pcie_2g_reset,  val);
			return 0;
		}
        	gpio_set_value_cansleep(port->pcie_2g_reset, 0);
	}
	port->pcie_5g_reset = of_get_named_gpio(node, "rst-5g-gpio", 0);
	if (gpio_is_valid(port->pcie_5g_reset)) {
		int val;
		val = gpio_request_one(port->pcie_5g_reset,
				GPIOF_OUT_INIT_LOW, "WIFI Reset");
		if ( val ) {
			pr_err("Could not request %d enable gpio: %d\n",
				port->pcie_5g_reset,  val);
			return 0;
		}
        	gpio_set_value_cansleep(port->pcie_5g_reset, 0);
	}
	return 0;
}
