...
void pm_spi_shutdown_notify_m4(void)
{
	struct pm_rpmsg_data msg;
...
void pm_spi_reboot_notify_m4(void)
{
	struct pm_rpmsg_data msg;
...
void  pm_spi_heartbeat_off_notify_m4(bool enter)
{
	struct pm_rpmsg_data msg;
...
static void pm_heart_beat_work_handler(struct work_struct *work)
{
	...
	if (pm_rpmsg.first_flag) {
		...
		pm_spi_heartbeat_off_notify_m4(heartbeat_off);
...
static struct rpmsg_device_id pm_rpmsg_id_table[] = {
	{ .name	= "rpmsg-pm-channel" },
	{ },
};

static struct rpmsg_driver pm_rpmsg_driver = {
	.drv.name	= "pm_spi_rpmsg",
