...
static const struct imx7ulp_pm_socdata imx7ulp_lpddr3_pm_data __initconst = {
	...
	.script = imx7ulp_lpddr3_script,
};

/* Sonos Trident LPDDR2 */
static const u32 trident_mmdc_io_lpddr2_offset[] __initconst = {
	0x0,  0x4,  0x8,  0xc,
	0x10, 0x14, 0x18, 0x1c,
	0x20, 0x24, 0x28, 0x2c,
	0x30, 0x34, 0x38, 0x3c,
...
static const u32 trident_mmdc_lpddr2_offset[] __initconst = {
	0x01C, 0x800, 0x85C, 0x890,
	0x848, 0x850, 0x81C, 0x820,
	0x824, 0x828, 0x8C0, 0x8B8,
	0x004, 0x00C, 0x010, 0x038,
	0x014, 0x018, 0x02C, 0x030,
	0x040, 0x000, 0x01C, 0x01C,
	0x01C, 0x01C, 0x01C, 0x01C,
	0x83C, 0x83C, 0x020, 0x800,
	0x004, 0x404, 0x01C,
};

static const u32 trident_lpddr2_script[] __initconst = {
	0x00008000, 0xA1390003, 0x1B4380BD, 0x00480000,
	0x3E3E3A3E, 0x2C2E2C2C, 0x33333333, 0x33333333,
	0x33333333, 0x33333333, 0x24922492, 0x00000800,
	0x00020064, 0x31354135, 0xB6AF0A83, 0x00160667,
	0x00C70093, 0x00201708, 0x0F9F26D2, 0x009F0E10,
	0x00000037, 0x83010000, 0x00008010, 0x003F8030,
	0xFF0A8030, 0x82018030, 0x06028030, 0x04038030,
	0xA0000000, 0xA0000000, 0x00001000, 0xA1310003,
	0x00025564, 0x00010106, 0x00000000,
};

static const struct imx7ulp_pm_socdata trident_lpddr2_pm_data __initconst = {
	.mmdc_compat = "fsl,imx7ulp-mmdc",
	.mmdc_io_num = ARRAY_SIZE(trident_mmdc_io_lpddr2_offset),
	.mmdc_io_offset = trident_mmdc_io_lpddr2_offset,
	.mmdc_num = ARRAY_SIZE(trident_mmdc_lpddr2_offset),
	.mmdc_offset = trident_mmdc_lpddr2_offset,
	.script = trident_lpddr2_script,
};

/* sequence used as index for the fsl,ddr_cfg_idx property */
static const struct imx7ulp_pm_socdata* socdata_array[] =
{
	&imx7ulp_lpddr3_pm_data,
	&trident_lpddr2_pm_data
};

/* pointer to hold the memory configuration selection */
static const struct imx7ulp_pm_socdata *socdata;
				      ...
				      int depth, void *data)
{
	...
	if (of_flat_dt_match(node, low_power_ocram_match)) {
		const __be32 *reg_prop = of_get_flat_dt_prop(node, "reg", NULL);
		const __be32 *idx_prop = of_get_flat_dt_prop(node, "fsl,ddr-cfg-idx", NULL);

		if (!reg_prop)
			return -EINVAL;

		lpram_addr = be32_to_cpup(reg_prop);
		...
		if (idx_prop)
			ddr_cfg_idx = be32_to_cpu(*idx_prop);
		if (!socdata && ddr_cfg_idx < ARRAY_SIZE(socdata_array))
			socdata = socdata_array[ddr_cfg_idx];
		pr_info("fsl,ddr-cfg-idx: %d\n", ddr_cfg_idx);
		pr_info("fsl,ddr-cfg-idx: %s\n", socdata->mmdc_compat);
	}
				...
				*socdata)
{
	...
	for (i = 0; i < pm_info->mmdc_num; i++)
		pm_info->mmdc_val[i][1] = socdata->script[i];
...
{
	imx7ulp_pm_common_init(socdata);
	imx7ulp_set_lpm(RUN);
}
