...
static int squashfs_page_offset(struct page *page, struct address_space *mapping)
{
	struct inode *inode = mapping->host;
	struct squashfs_sb_info *msblk = inode->i_sb->s_fs_info;
	int file_end = i_size_read(inode) >> msblk->block_log;
        int index = page->index >> (msblk->block_log - PAGE_SHIFT);
	int offset = index == file_end ?
			(i_size_read(inode) & (msblk->block_size - 1)) :
			 msblk->block_size;
	return offset;
}
/* Read datablock stored packed inside a fragment (tail-end packed block) */
static int squashfs_readpage_fragment(struct page *page, int offset)
{
	struct inode *inode = page->mapping->host;
	...
	if (res)
		...
		squashfs_copy_cache(page, buffer, offset,
       ...
       struct list_head *readahead_pages, struct address_space *mapping)
{
	...
	offset = squashfs_page_offset(page, mapping);

	return squashfs_readpage_fragment(page, offset);
}


static int squashfs_readpage_sparse(struct page *page, int offset)
{
	squashfs_copy_cache(page, NULL, offset, 0);
	return 0;
}
	...
	struct address_space *mapping)
{
	...
	offset = squashfs_page_offset(page, mapping);
	return squashfs_readpage_sparse(page, offset);
}
